...
# in_date = "20191027" # "20020224" #"20210721" # "20210719" corresponds to slurm task array 200
# fldr_zarr_fullres_daily = "/scratch/dcl3nd/highres-radar-rainfall-processing/_scratch/mrms_zarr_preciprate_fullres_dailyfiles/"
...
try:
    ...
    print("Bias correcting MRMS dataset over input shapefile........")
except:
    f_shp_sst_transom = None
...
if not overwrite_existing_outputs:
    try:
        ...
        check2 = True
        ds_out = xr.open_dataset(fl_out_zarr, engine = "zarr")
        result = check_duplicates(ds_out, dims=['latitude', "longitude"])
        if result == "duplicates":
            print("Invalid spatial coordinates for previously output dataset. Reprocessing....")
            check2 = False
        ...
        pass
if check and check2:
    print(f"Output already created with no errors and overwrite_existing_outputs is set to {overwrite_existing_outputs}. Not re-running. Output filepath: {fl_out_zarr}")
    sys.exit(0)
else:
...
tmp_raw_mrms_zarr = fldr_scratch_zarr + in_date + "_raw.zarr"
...
performance["problems_loading_raw_mrms_data"] = "no_problems"
try:
    ds_mrms = xr.open_dataset(tmp_raw_mrms_zarr, chunks = dic_chunks_mrms, engine = "zarr")
except Exception as e:
    print("###########################")
    print(f"There were problems loading the raw mrms data. Filepath: {tmp_raw_mrms_zarr}")
    print(f"Error: {e}")
    print("###########################")
    performance["problems_loading_raw_mrms_data"] = e
...
performance["problems_loading_data_for_bias_correction"] = "no_problems"
try:
    ...
    print("#####################################")
    print(f"Trouble loading file {performance['filepath_ref_data']}")
    print(e)
    print("#####################################")
    performance["data_available_for_bias_correction"] = False
    performance["problems_loading_data_for_bias_correction"] = e

# sort the dimensions of the mrms and reference dataset
for dim in ds_mrms.dims:
    ds_mrms = ds_mrms.sortby(dim)
    ds_ref = ds_ref.sortby(dim)

if performance["data_available_for_bias_correction"] and (performance["problems_loading_raw_mrms_data"] == "no_problems"):
    if gdf_transdomain is not None:
...
if (performance["problems_loading_raw_mrms_data"] == "no_problems"):
    try:
        ...
        print("#####################################")
        print("Problem exporting final zarr")
        print(e)
        print("#####################################")
...
for i, f in enumerate(lst_tmp_files_to_delete):
    try:
        shutil.rmtree(f)
    except Exception as e:
        print("#####################################")
        print(f"File index {i} in lst_tmp_files_to_delete could not be removed. Filepath: {f}")
        print(f"Full list: {lst_tmp_files_to_delete}")
        print(f"Error: {e}")
        print("#####################################")
